...
def get_gcp_pg_table_chk_ht(chk_dt):
    sql_query = f"""
        ...
        from 
            comm_df.gcp_pg_table_chk_ht ht, comm_df.clct_table_mt ctm, comm_df.db_mt pdimm 
        where
            ht.diff_cnt < 0 and
            ...
            chk_dte >= '{chk_dt}'
            and work_yn = 'N'
            and (bq_dataset_nm like '%_tag' or bq_table_nm like '%l2_mid_trk_mt' or bq_table_nm like 'tms_%')
        order by 
...
def load_data_from_postgres(*args, **kwargs):
    ...
    df_chk_plc = df_chk_history.loc[df_chk_history.bq_dataset_nm.str.contains('_tag') | df_chk_history.bq_dataset_nm.str.contains('_tms')]
